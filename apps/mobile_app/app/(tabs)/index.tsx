...
//   );
// }
// ── PAYTM FLOW — commented out, keep for future ───────
...
//     const callbackUrl = String(data.callbackUrl).trim();
//     await AllInOneSDKManager.startTransaction(
//       orderId, mid, txnToken, amount, callbackUrl, true, true, `paytm${mid}`
//     );
//     try {
//       const verifyRes = await api.post('/payment/verify', { orderId });
//       if (verifyRes.data.data.status === 'SUCCESS') {
//         showMessage('success', 'Payment Successful', 'Your payment has been completed successfully.', {
//           showCancel: false, confirmText: 'OK',
//         });
//       } else {
//         showMessage('error', 'Payment Failed', `${verifyRes.data.data.message}.`, {
//           showCancel: false, confirmText: 'OK',
//         });
//       }
//     } catch (verifyError: any) {
//       const msg = verifyError?.response?.data?.message || verifyError?.message
//         || 'Your payment was successful. We are confirming it in the background.';
//       showInfo('Payment Processing', msg);
//     }
//   } catch (error: any) {
//     const msg = error?.response?.data?.message || error?.message || 'Please try again after some time.';
...
export default function HomeScreen() {
  ...
  const rankMeta = getRankMeta(user?.membersCount || 0);
  const count = user?.membersCount || 0;
  const thresholds = [0, 5, 25, 125, 625, 3125];
  const rankIdx = thresholds.findIndex((t, i) => count < (thresholds[i + 1] ?? Infinity));
  const current = thresholds[rankIdx] ?? 625;
  const next = thresholds[rankIdx + 1];
  const progress = next ? (count - current) / (next - current) : 1;
